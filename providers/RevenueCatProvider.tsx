...
const initialize = useCallback(
  async (appUserId?: string | null) => {
    ...
    try {
      ...
      try {
        if (typeof (loadedPurchases as any).setAttributes === "function") {
          await (loadedPurchases as any).setAttributes({ app_build: "expo" });
        }
      } catch (e) {
...
useEffect(() => {
  ...
  try {
    if (typeof (purchases as any).addCustomerInfoUpdateListener === "function") {
  ...
  return () => {
    try {
      if (typeof (purchases as any).removeCustomerInfoUpdateListener === "function") {
...
const {
  ...
  queryFn: async () => {
    ...
    if (typeof (loadedPurchases as any).getOfferings !== "function") {
      return undefined;
    }
    const offerings = (await (loadedPurchases as any).getOfferings()) as PurchasesOfferings;
...
const {
  ...
  queryFn: async () => {
    ...
    if (typeof (loadedPurchases as any).getCustomerInfo !== "function") {
      return undefined;
    }
    const info = (await (loadedPurchases as any).getCustomerInfo()) as CustomerInfo;
...
useEffect(() => {
  ...
  (async () => {
    ...
    try {
      const result = (await (loadedPurchases as any).checkTrialOrIntroductoryPriceEligibility(
        identifiers
...
const { mutateAsync: purchasePackageAsync, isPending: isPurchasePending } = useMutation({
  mutationFn: async (pkg: PurchasesPackage) => {
    ...
    if (typeof (loadedPurchases as any).purchasePackage !== "function") {
      throw new Error("RevenueCat purchasePackage unavailable");
    }
    const { customerInfo: info } = (await (loadedPurchases as any).purchasePackage(pkg)) as {
      customerInfo?: CustomerInfo;
    };
    return (info ?? null) as CustomerInfo;
  },
  onSuccess: (info) => {
    queryClient.setQueryData(["revenuecat", "customer-info", purchasesKeyPart], info);
    setLastError(null);
...
const { mutateAsync: restorePurchasesAsync, isPending: isRestorePending } = useMutation({
  mutationFn: async () => {
    ...
    const loadedPurchases = await getPurchases();
    if (!loadedPurchases) throw new Error("RevenueCat unavailable");
    if (typeof (loadedPurchases as any).restorePurchases !== "function") {
      throw new Error("RevenueCat restorePurchases unavailable");
    }
    return (loadedPurchases as any).restorePurchases() as Promise<CustomerInfo>;
  },
  onSuccess: (info) => {
    queryClient.setQueryData(["revenuecat", "customer-info", purchasesKeyPart], info);
    setLastError(null);
...
const refreshCustomerInfo = useCallback(async () => {
  ...
  const loadedPurchases = await getPurchases();
  if (!loadedPurchases) return;
  if (typeof (loadedPurchases as any).invalidateCustomerInfoCache === "function") {
    await (loadedPurchases as any).invalidateCustomerInfoCache();
  }
  await refetchCustomerInfo();
    ...
    try {
      ...
      if (result?.customerInfo) {
        queryClient.setQueryData(
          ["revenuecat", "customer-info", purchasesKeyPart],
          result.customerInfo
        );
        setLastError(null);
      }
